...
export default function CityAddPage() {
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const onSubmit = async (data) => {
    ...
    const newCity = await CitiesService.add(data);
    setFormSubmitting(false);
    if (newCity) {
      navigate(`/admin/cities?countryId=${newCity.countryId}`);
      toastify.success(t("SUCCESSFULLY_ADDED_CITY"));
    } else {
      toastify.error(t("ERROR_ADDING_CITY"));
    }
  };
  return (
    <>
      <div className="row">
        <div className="col-12 mb-4 mb-sm-5">
          <h1 className="h3 mb-0">{t("NEW_CITY")}</h1>
        </div>
      </div>
